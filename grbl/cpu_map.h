...
*/
  ...
  #if !defined(DUAL_AXIS_CUSTOM_SHIELD)
    // Define user-control controls (cycle start, reset, feed hold) input pins.
    // NOTE: All CONTROLs pins must be on the same port and not on a port with other input pins (limits).
    #define CONTROL_DDR       DDRC
    #define CONTROL_PIN       PINC
  ...
  // Define the coolant enable output pins.
  // NOTE: Not used on the custom shield, but defined to prevent
  // coolant control module from barking at us
  #define COOLANT_FLOOD_DDR   DDRJ
  #define COOLANT_FLOOD_PORT  PORTJ
  #define COOLANT_FLOOD_BIT   6

  // Define user-control controls (cycle start, reset, feed hold) input pins.
  // NOTE: All CONTROLs pins must be on the same port and not on a port with other input pins (limits).
  // NOTE: Not used on the custom shield, but defined to prevent
  // system module from barking at us
